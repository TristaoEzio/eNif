...
suite('Render Functions', () => {
	...
	suite('renderADML', () => {
		...
		test('should render ADML with correct XML structure', () => {
			const result = renderADML('eNif', ['1.85'], [mockCategory], [mockPolicy]);
		...
		test('should include supported versions with escaped greater-than', () => {
			const result = renderADML('eNif', ['1.85', '1.90'], [mockCategory], [mockPolicy]);

			assert.ok(result.includes('eNif &gt;= 1.85'));
			assert.ok(result.includes('eNif &gt;= 1.90'));
		});

		test('should include category strings', () => {
			const result = renderADML('eNif', ['1.0'], [mockCategory], [mockPolicy]);
		...
		test('should pass translations to policy strings', () => {
			...
			const result = renderADML('eNif', ['1.0'], [mockCategory], [mockPolicy], translations);
		...
		test('should handle multiple categories', () => {
			...
			const result = renderADML('eNif', ['1.0'], [category1, category2], [mockPolicy]);
...
<string>${translations?.['testModule']?.['test.desc'] || 'Default Desc'}</string>
		...
		test('should render profile manifest with correct XML structure', () => {
			const result = renderProfileManifest('eNif', 'com.microsoft.vscode', ['1.0'], [mockCategory], [mockPolicy]);
		...
		test('should pass translations to policy manifests', () => {
			...
			const result = renderProfileManifest('eNif', 'com.microsoft.vscode', ['1.0'], [mockCategory], [mockPolicy], translations);

			assert.ok(result.includes('Translated Description'));
		});

		test('should include eNif specific URLs', () => {
			const result = renderProfileManifest('eNif', 'com.microsoft.vscode', ['1.0'], [mockCategory], [mockPolicy]);
...
<string>TestPolicy2</string>
			...
			const result = renderProfileManifest('eNif', 'com.microsoft.vscode', ['1.0'], [mockCategory], [mockPolicy, policy2]);
...
<plist version="1.0">
	<dict>
		...
		<string>This profile manages eNif. For more information see https://code.visualstudio.com/docs/setup/enterprise</string>
		<key>PayloadDisplayName</key>
		<string>eNif</string>
		<key>PayloadIdentifier</key>
		<string>com.microsoft.vscode</string>
		...
		test('should include en-us manifest by default', () => {
			...
			assert.ok(result.manifests[0].contents.includes('eNif Managed Settings'));
		});
	...
	suite('renderGP', () => {
		...
		test('should include en-us ADML by default', () => {
			...
			assert.ok(result.adml[0].contents.includes('eNif'));
		});
		...
		test('should pass versions to ADML', () => {
			...
			assert.ok(result.adml[0].contents.includes('eNif &gt;= 1.85'));
		});
