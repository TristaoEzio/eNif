...
/**
 * Converts eNif build platform/arch to the values that Node.js reports
...
function toNodePlatformArch(platform: string, arch: string): { nodePlatform: string; nodeArch: string } {
	...
	if (arch === 'armhf') {
		// eNif build uses 'armhf'; Node reports process.arch === 'arm'
		nodeArch = 'arm';
	} else if (arch === 'alpine') {
 ...
 * resolves `node-pty` from the embedder (eNif) first via `hostRequire`,
...
export function getCopilotExcludeFilter(platform: string, arch: string): string[] {
	...
	// resolves `node-pty` from eNif's own node_modules via `hostRequire`.
 ...
 * `node-pty` from the embedder (eNif) via `hostRequire` and falls back to
