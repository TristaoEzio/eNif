...
async function main(buildDir?: string, skipProvisioningProfile?: boolean): Promise<void> {
	...
	if (arch !== 'universal' && !skipProvisioningProfile) {
		await spawn('plutil', [
			...
			'-string',
			'An application in eNif wants to use AppleScript.',
			`${infoPlistPath}`
		]);
		await spawn('plutil', [
			'-replace',
			...
			'-string',
			'An application in eNif wants to use the Microphone.',
			`${infoPlistPath}`
		]);
		await spawn('plutil', [
			'-replace',
			...
			'-string',
			'An application in eNif wants to use the Camera.',
			`${infoPlistPath}`
		]);
		await spawn('plutil', [
			'-replace',
			...
			'-string',
			'An application in eNif wants to use Audio Capture.',
			`${infoPlistPath}`
		]);
		await spawn('plutil', [
			'-insert',
