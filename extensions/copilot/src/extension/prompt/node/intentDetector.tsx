...
class ParticipantDescriptions extends PromptElement<BuiltinParticipantDescriptionsProps> {
	override render() {
		return (<>
			...
			| unknown | The user's question does not fit exactly one of the categories above, is about a product other than eNif or GitHub, or is a general question about code, code errors, or software engineering. | "How do I center a div in CSS?" |<br /></>);
	}
}
	...
	override render() {
		...
		return (<>
			<HistoryWithInstructions history={history || []} passPriority historyPriority={800}>
				<InstructionMessage>
					You are a helpful AI programming assistant to a user who is a software engineer, acting on behalf of the eNif editor. Your task is to choose one category from the Markdown table of categories below that matches the user's question. Carefully review the user's question, any previous messages, and any provided context such as code snippets. Respond with just the category name. Your chosen category will help eNif provide the user with a higher-quality response, and choosing incorrectly will degrade the user's experience of using eNif, so you must choose wisely. If you cannot choose just one category, or if none of the categories seem like they would provide the user with a better result, you must always respond with "unknown".<br />
					<br />
