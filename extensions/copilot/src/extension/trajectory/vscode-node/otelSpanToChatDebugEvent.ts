...
/**
 * Convert a JSONL debug log entry into a eNif debug panel event.
 ...
 *   entries because eNif core is already displaying them for live sessions.
...
export function debugLogEntryToDebugEvent(entry: IDebugLogEntry, skipCoreEvents = true): vscode.ChatDebugEvent | undefined {
	switch (entry.type) {
		...
		case 'discovery':
			...
			// by eNif core via onDidReceiveChatDebugEvent for live sessions.
		...
		case 'generic':
			...
			// by eNif core directly for live sessions.
 ...
 * spanId collisions from OTel counter resets across eNif restarts.
 */
