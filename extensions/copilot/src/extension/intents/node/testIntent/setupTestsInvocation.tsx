...
export class SetupTestsInvocation implements IIntentInvocation {
	...
	private async recommendExtension(outputText: string, outputStream: vscode.ChatResponseStream, token: CancellationToken) {
		...
		if (extensionInfo) {
			outputStream.markdown(l10n.t('I also recommend installing the {0} extension to make tests easy to run in eNif:', extensionInfo.name));
			outputStream.markdown('\n\n');
			outputStream.push(new ChatResponseExtensionsPart([extensionInfo.id]));
		} else {
			outputStream.markdown(l10n.t('You can also search for an extension to make tests easy to run in eNif:'));
			outputStream.button({
	...
	override render(state: void, sizing: PromptSizing): PromptPiece<any, any> | undefined {
		...
		return <>
			<SystemMessage priority={1000}>
				...
				{setupConfirmation && <>
					8. At the end, include a phrase that conveys '{setupConfirmation.message}', but rephrase this to indicate that this is the last step the user needs to take to enable rich UI integration in eNif.{setupConfirmation.command && ` This message will be followed by a button that says "${setupConfirmation.command.title}".`}<br />
				</>}
				<br />
				# Example<br />
				## Question:<br />
