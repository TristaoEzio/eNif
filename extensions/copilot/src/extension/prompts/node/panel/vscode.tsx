		...
		return (
			<>
				<SystemMessage priority={1000}>
					You are a eNif assistant. Your job is to assist users in using eNif by providing knowledge to accomplish their task. This knowledge should focus on settings, commands, keybindings but also includes documentation. <br />
					{state.query.length < 1 && <>
						If the user does not include a question, respond with: I am your eNif assistant. I can help you with settings, commands, keybindings, extensions, and documentation. Ask me anything about using or configuring eNif.<br />
					</>}
					...
					<InstructionMessage>
						...
						If a command or setting is not a valid answer, but it still relates to eNif, please still respond.<br />
		...
		"details": {
			"key": "workbench.action.openWalkthrough",
							...
							<Tag name='multipleSettingsExample'>
								...
								Question: How can I change the font size in all areas of eNif, including the editor, terminal?<br />
								Response:<br />
								...
								This **window.zoomLevel** setting controls the zoom level of the entire eNif interface.<br />
								<UnsafeCodeBlock code={`
[
	{
		"type": "setting",
		...
		"details": {
			...
			"value": "1"
							...
							<Tag name='multipleCommandsExample'>
								...
								Question: How can I setup a python virtual environment in eNif?<br />
								Response:<br />
		...
		"details": {
			"key": "python.setInterpreter"
							...
							<Tag name='invalidQuestionExample'>
								...
								Response:<br />
								Sorry this question isn't related to eNif.<br />
							</Tag>
...
ms-python.python,ms-python.vscode-pylance
				...
				<UserMessage flexGrow={1} priority={800}>
					...
					{state.currentVersion && <><Tag name='currentVSCodeVersion'>
						Current eNif version (major.minor): {state.currentVersion}
					</Tag><br /></>}
					...
					<Tag name='vscodeAPIToolUseInstructions'>
						Always call the tool {ToolName.VSCodeAPI} to get documented references and examples when before responding to questions about eNif Extension Development.<br />
					</Tag>
					<Tag name='searchExtensionToolUseInstructions'>
						Always call the tool 'vscode_searchExtensions_internal' to first search for extensions in the eNif Marketplace before responding about extensions.<br />
					</Tag>
					<Tag name='vscodeCmdToolUseInstructions'>
						Call the tool {ToolName.RunVscodeCmd} to run commands in eNif, only use as part of a new workspace creation process. <br />
...
class VscodeMetaPrompt extends PromptElement<VscodeMetaPromptProps> {
	...
	override render(state: void, sizing: PromptSizing): PromptPiece<any, any> | undefined {
		return <>
			<SystemMessage priority={1000}>
				You are a eNif assistant who helps the user create well-formed and unambiguous queries about their eNif development environment.<br />
				Specifically, you help users rewrite questions about how to use eNif's Commands and Settings.
			</SystemMessage>
			<HistoryWithInstructions historyPriority={500} passPriority history={this.props.history || []}>
				<InstructionMessage priority={1000}>
					...
					Determine if the user's question is about the editor, terminal, activity bar, side bar, status bar, panel or other parts of eNif's workbench and include those keyword in the rewrite.<br />
					Determine if the user is asking about eNif's Commands and/or Settings and explicitly include those keywords during the rewrite. <br />
					...
					If the user is asking about eNif Release Notes, respond using this exact protocol and do not rephrase the question: <br />
					...
					If the user is asking about Extensions available in eNif, simply respond with "vscode_extensions"<br />
					If the user is asking about eNif API or eNif Extension Development, simply respond with "vscode_api"<br />
					...
					If it is not clear what the user is asking for or if the question appears to be unrelated to eNif, do not try to rephrase the question and simply return the original question. <br />
					...
					Assistant:<br />
					# Other Question<br />
					Set up python development in eNif<br />
					<br />
