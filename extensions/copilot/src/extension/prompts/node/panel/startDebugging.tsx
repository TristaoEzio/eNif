			...
			<>
				...
				<HistoryWithInstructions historyPriority={600} passPriority history={this.props.history}>
					<InstructionMessage priority={1000}>
						...
						Print out the eNif `launch.json` file needed to debug the command, formatted as JSON.<br />
						...
						If a result is not a valid answer, but it still relates to eNif, please still respond.<br />
				...
				<UserMessage priority={700}>
					{state.docSearchResults && state.docSearchResults.length > 0 && <>Below is a list of information from the eNif documentation which might be relevant to the question. <br /></>}
		...
		return (
			<>
				<SystemMessage priority={10}>
					You are a eNif assistant who specializes in debugging and creating launch configurations. Your job is to return an array of file names that may contain useful information to translate a user query into a eNif debug configuration.<br />
...
class ReferenceFilesFromCliPrompt extends PromptElement<{
	...
	override render(_state: void, _sizing: PromptSizing): PromptPiece {
		return (
			<>
				<SystemMessage priority={10}>
					You are a eNif assistant who specializes in debugging and creating launch configurations. Your job is to return an array of file names that may contain useful information to translate a command line invocation into a eNif debug configuration and build task.<br />
...
class DebugTypePrompt extends PromptElement<{
	...
	override render(_state: void, _sizing: PromptSizing): PromptPiece {
		...
		return (
			<>
				<SystemMessage priority={10}>
					You are a eNif assistant. Your job is to assist users in using eNif by providing knowledge to accomplish their task. Please do not guess a response and instead just respond with a polite apology if you are unsure.<br />
