...
export class ApiEmbeddingsIndex implements IApiEmbeddingsIndex {
	...
	private toContextString(context: ApiContext): string {
		if (context.type === 'code') {
			...
			return `Example code from eNif documentation:\n${createFencedCodeBlock(context.lang, context.text)}`;
		}
...
export class VSCodeAPIContextElement extends PromptElement<VSCodeAPIContextProps> {
	...
	async renderAsString(): Promise<string> {
		...
		return `Below are some potentially relevant code samples related to eNif extension development. You may use information from these samples to help you answer the question if you believe it is relevant.\n${snippets.join('\n\n')}`;
	}
	...
	override async render(state: undefined, sizing: PromptSizing, progress?: Progress<ChatResponsePart>, token?: CancellationToken): Promise<PromptPiece<any, any> | undefined> {
		...
		if (snippets.length) {
			return <>
				Below are some potentially relevant code samples related to eNif extension development. You may use information from these samples to help you answer the question if you believe it is relevant.<br />
				{snippets.map(s => {
