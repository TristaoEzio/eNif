...
This folder contains the Copilot CLI integration for eNif Chat. It enables users to open a new Chat window and interact with a Copilot CLI agent instance directly within eNif. **eNif provides the UI, Copilot CLI SDK provides the smarts.**
...
│  - Provides eNif-specific tools to the SDK via MCP protocol   │
...
The integration follows eNif's platform layering pattern with three layers:

```
copilotcli/
├── common/                     # Platform-agnostic (NO Node.js or eNif API imports)
...
│   ├── ripgrepShim.ts          # Copies eNif's ripgrep for SDK use
│   └── test/
│
└── vscode-node/                # eNif API-dependent (commands, MCP tools, UI)
...
- Registers a `LanguageModelChatProvider` with `targetChatSessionType: 'copilotcli'` so eNif's model picker shows CLI models
...
- Listens to SDK events and translates them to eNif chat UI parts
...
- Proxies all eNif-configured MCP servers through a gateway URL with `type: 'http'` config per server
...
Persists eNif-specific metadata that sits alongside the SDK's own session data. This metadata is **not part of the SDK's `events.jsonl`** — it tracks eNif concepts like worktree properties, request-to-tool mappings, mode instructions, and checkpoint refs.
...
- **Shims before SDK import**: `ensureRipgrepShim()` in `node/ripgrepShim.ts` MUST be called before any `import('@github/copilot/sdk')`. It copies eNif's bundled ripgrep binary to the SDK's expected location. `node-pty` is no longer shimmed: the copilot CLI SDK resolves it from eNif's own `node_modules` via `hostRequire`, falling back to its bundled copy only if that fails. See `node/copilotCli.ts` for the initialization order.
...
**eNif Session commands** (registered via `registerCLIChatCommands` in `vscode-node/copilotCLIChatSessions.ts`):

## Configuration

The integration respects these eNif settings (all under `github.copilot.chat.cli.*`):
...
| `showExternalSessions` | `false` | Show sessions created outside eNif (e.g., terminal CLI) |
