...
/**
 * Build chat history from SDK events for eNif chat session
 ...
 * eNif {@link McpToolInvocationContentData}[] objects for rendering in the chat UI.
 *
 ...
 * content shapes into a common binary+MIME-type representation that the eNif chat
...
export function createCopilotCLIToolInvocation(data: {
	...
	if (toolCall.toolName === 'show_file') {
		...
		// Its a WIP, no clear way to render in CLI either, hence decided to hide in eNif.
		return undefined;
	}
