...
/**
 *
 * We keep parsing in two phases to mirror eNif's shape:
 ...
 * workspace root. eNif's built-in detector cannot resolve that context.
 */
export class CopilotCLITerminalLinkProvider implements TerminalLinkProvider<CopilotCLITerminalLink> {
	...
	async provideTerminalLinks(context: TerminalLinkContext, token: CancellationToken): Promise<CopilotCLITerminalLink[]> {
		...
		// Match eNif's built-in MaxLineLength limit (terminalLocalLinkDetector.ts).
		...
		for (const candidate of this._detectLinkCandidates(line)) {
			// Match eNif's built-in MaxResolvedLinksInLine (terminalLocalLinkDetector.ts).
			...
			// Mirrors eNif's specialEndCharRegex (terminalLocalLinkDetector.ts).
			let trimmed = 0;
