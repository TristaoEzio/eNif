...
/**
 * Converts a Claude Code session into eNif chat history turns.
 *
 ...
 * appears as alternating assistant/user messages in the JSONL. eNif's chat
...
export function buildChatHistory(session: IClaudeCodeSession): (vscode.ChatRequestTurn2 | vscode.ChatResponseTurn2)[] {
	...
	while (i < messages.length) {
		...
		if (currentType === 'user') {
			...
			// as a separate ChatResponseTurn2 because the eNif chat widget
