...
export interface IClaudeSlashCommandService {
	 ...
	 *
	 * Checks `request.command` first (eNif slash command), then falls back to
...
export class ClaudeSlashCommandService extends Disposable implements IClaudeSlashCommandService {
	...
	constructor(
		...
		super();
		// Initialize eagerly to register eNif commands at startup
	...
	async tryHandleCommand(
		...
		// 1. Check request.command (eNif slash command selected via UI)
		if (request.command) {
	...
	private _ensureInitialized(): void {
		...
		for (const ctor of ctors) {
			...
			// Register eNif command if commandId is provided
