...
This folder contains the Claude Code integration for eNif Chat. It enables users to open a new Chat window and interact with a Claude Code instance directly within eNif. **eNif provides the UI, Claude Code provides the smarts.**
...
The Claude Code integration allows eNif's chat interface to communicate with Claude Code, Anthropic's agentic coding assistant. When a user sends a message in a eNif Chat window using this integration, the message is routed to a Claude Code session that can:
...
All interactions are displayed through eNif's native chat UI, providing a seamless experience.
...
**ClaudeAgentManager**
- Entry point for handling chat requests from eNif
...
- Resolves prompts by replacing eNif references (files, locations) with actual paths

**ClaudeCodeSession**
- Represents a single Claude Code conversation session
- Manages a queue of incoming requests from eNif Chat
...
- Handles tool confirmation dialogs via eNif's chat API
...
Formats tool invocations for display in eNif's chat UI:
...
Converts a persisted `IClaudeCodeSession` into eNif `ChatResponsePart[]` for replay in the chat UI:
...
1. **User sends message** in eNif Chat
...
4. **Claude Code SDK** returns streaming messages:
   ...
   - Tool use requests → shown as progress, then confirmed via eNif's confirmation API
...
eNif calls `getChatSessionInputState` to get a `ChatSessionInputState` object whose `.groups` array drives the UI. Rather than computing groups once and returning them, the pipeline keeps `groups` live: shared observables push changes into each state object whenever relevant configuration changes.
...
When eNif discards a `ChatSessionInputState`, the `onDidDispose` event fires and deterministically cleans up all autoruns for that state. The `onDidDispose` subscription is itself registered on the pipeline store, so it is cleaned up as part of disposal.
...
- **`context.previousInputState` path** — eNif already has a state for this session and is asking for a fresh one; seed from the old groups.
...
- Commands can optionally register with eNif Command Palette
...
interface IClaudeSlashCommandHandler {
	...
	readonly commandId?: string;          // Optional eNif command ID
...
- All other tools show a confirmation dialog via eNif's chat API
...
- `vscode-node/mcpServers/index.ts` → eNif-specific contributors (imports node first)
...
The integration respects eNif settings:
