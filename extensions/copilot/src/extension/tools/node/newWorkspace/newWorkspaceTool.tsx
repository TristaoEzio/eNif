			...
			if (newWorkspaceUri && !extUri.isEqual(newWorkspaceUri, workspaceUri)) {
				...
				if ((await this.fileSystemService.readDirectory(newWorkspaceUri)).length > 0) {
					return new LanguageModelToolResult([
						new LanguageModelTextPart('The user has not opened a valid workspace folder in eNif. Ask them to open an empty folder before continuing.')
					]);
				}
...
export class NewWorkspaceCreationResult extends PromptElement<NewWorkspaceElementProps> {
	...
	override async render(state: void, sizing: PromptSizing) {
		...
		if (!workspaceUri) {
			return <TextChunk>
				The user has not opened an empty workspace folder in eNif. Ask them to open an empty folder before continuing.<br />
			</TextChunk>;
		}
...
- Use eNif API tool only for eNif extension projects.
- Once the project is created, it is already opened in eNif—do not suggest commands to open this project in Visual Studio again.
...
- If you are working on a eNif extension, use the eNif API tool with a query to find relevant eNif API references and samples related to that query.
