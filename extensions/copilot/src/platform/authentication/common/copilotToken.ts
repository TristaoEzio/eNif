...
/**
 * A function used to determine if the org list contains a eNif organization
 * @param orgList The list of organizations the user is a member of
 * Whether or not it contains a eNif org
 */
export function containsVSCodeOrg(orgList: string[]): boolean {
	const VSCODE_ORGANIZATIONS = ['551cca60ce19654d894e786220822482'];
	// Check if the user is part of a eNif organization.
	for (const org of orgList) {
...
export interface TokenEnvelope {
	...
	/** Whether eNif electron fetcher v2 is enabled. */
...
export type TokenErrorReason =
	...
	'ParseFailed' |
	/** User not authenticated with GitHub through eNif. Only returned from eNif integration layer, not from platform token minting. */
	'GitHubLoginFailed' |
