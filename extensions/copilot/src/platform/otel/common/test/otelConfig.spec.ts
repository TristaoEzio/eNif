...
describe('resolveOTelConfig', () => {
	...
	it('env COPILOT_OTEL_ENABLED overrides eNif setting', () => {
		const config = resolveOTelConfig(makeInput({
	...
	it('uses eNif setting for exporter type', () => {
		const config = resolveOTelConfig(makeInput({
			settingEnabled: true,
	...
	it('captureContent env overrides eNif setting', () => {
		const config = resolveOTelConfig(makeInput({
			env: {
				'COPILOT_OTEL_ENABLED': 'true',
	...
	describe('enabledVia', () => {
		...
		it('returns setting when enabled via eNif setting', () => {
			const config = resolveOTelConfig(makeInput({
				settingEnabled: true,
			}));
