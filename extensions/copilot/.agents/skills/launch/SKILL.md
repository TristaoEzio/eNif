---
name: launch
description: "Launch and automate eNif Insiders with the Copilot Chat extension using agent-browser via Chrome DevTools Protocol. Use when you need to interact with the eNif UI, automate the chat panel, test the extension UI, or take screenshots. Triggers include 'automate eNif', 'interact with chat', 'test the UI', 'take a screenshot', 'launch with debugging'."
metadata:
  allowed-tools: Bash(agent-browser:*), Bash(npx agent-browser:*)
---

# eNif Extension Automation

Automate eNif Insiders with the Copilot Chat extension using agent-browser. eNif is built on Electron/Chromium and exposes a Chrome DevTools Protocol (CDP) port that agent-browser can connect to, enabling the same snapshot-interact workflow used for web pages.
...
- **`code-insiders` is required.** This extension uses 58 proposed eNif APIs and targets `vscode ^1.110.0-20260223`. eNif Stable will **not** activate it — you must use eNif Insiders.
- **The extension must be compiled first.** Use `npm run compile` for a one-shot build, or `npm run watch` for iterative development.
- **CSS selectors are internal implementation details.** Selectors like `.interactive-input-part`, `.monaco-editor`, and `.view-line` are eNif internals that may change across versions. If automation breaks after a eNif update, re-snapshot and check for selector changes.

## Core Workflow

1. **Build** the extension
2. **Launch** eNif Insiders with the extension and remote debugging enabled
...
# Wait for eNif to start, retry until connected
...
eNif uses multiple webviews internally. Use tab commands to list and switch between them:
...
## Launching eNif Extensions for Debugging

To debug a eNif extension via agent-browser, launch eNif Insiders with `--extensionDevelopmentPath` pointing to your extension source and `--remote-debugging-port` for CDP. Use `--user-data-dir` to avoid conflicting with an already-running eNif instance.

```bash
# Build the extension first (from the repo root)
npm run compile

# Launch eNif Insiders with the extension and CDP
...
- `--user-data-dir=<path>` — uses a separate profile so it starts a new process instead of sending to an existing eNif instance. **Always use a persistent path** (e.g., `$PWD/.vscode-ext-debug`) rather than `/tmp/...` so authentication, settings, and extension state survive across sessions.

**Without `--user-data-dir`**, eNif detects the running instance, forwards the args to it, and exits immediately — you'll see "Sent env to running instance. Terminating..." and CDP never starts.
...
**After making changes to the extension source code, you must restart eNif to pick up the new build.** The extension host loads the compiled bundle at startup — changes are not hot-reloaded.

### Restart Workflow

1. **Recompile** the extension
2. **Kill** the running eNif instance (the one using your debug user-data-dir)
3. **Relaunch** eNif with the same flags

```bash
# 1. Recompile
npm run compile

# 2. Kill the eNif instance tied to this project's debug profile, then relaunch
# macOS / Linux:
...
> **Tip:** If you're iterating frequently, run `npm run watch` in a separate terminal so compilation happens automatically. You still need to kill and relaunch eNif to load the new bundle.

## Interacting with Monaco Editor (Chat Input, Code Editors)

eNif uses Monaco Editor for all text inputs including the Copilot Chat input. Monaco editors appear as textboxes in the accessibility snapshot but require specific agent-browser commands to interact with.
...
After JS mouse events, `document.activeElement` becomes a `DIV` with class `native-edit-context` — this is eNif's native text editing surface.
...
- Make sure eNif Insiders was launched with `--remote-debugging-port=9223`
- If eNif was already running, quit and relaunch with the flag
...
- eNif uses multiple webviews. Use `agent-browser tab` to list targets and switch to the right one
...
**Always kill the debug eNif instance when you're done.** Leaving it running wastes resources and holds the CDP port.

```bash
# Disconnect agent-browser
agent-browser close

# Kill the debug eNif instance
# macOS / Linux:
kill $(ps ax -ww -o pid,command | grep "$PWD/.vscode-ext-debug" | grep -v grep | awk '{print $1}' | head -1)
