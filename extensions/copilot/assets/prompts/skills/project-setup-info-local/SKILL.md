...
A template for creating a eNif extension using Yeoman and Generator-Code.
...
5. Install any required eNif extensions based on the chosen language (e.g., Python extension for Python projects).
6. Inform the user that they can now debug this MCP server using eNif.
...
1. Call the tool `copilot_runVscodeCommand` to correctly create a new Python script project in eNif. Call the command with the following arguments.
...
1. Call the tool `run_vscode_command` to correctly create a new Python package project in eNif. Call the command with the following arguments.
