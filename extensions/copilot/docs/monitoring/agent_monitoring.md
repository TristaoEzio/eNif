...
- **Docker** installed
- **eNif** with the GitHub Copilot Chat extension
...
> **Note:** You can also use environment variables instead of eNif settings (see [Configuration](#configuration)). Environment variables always take precedence.
...
Environment variables **always take precedence** over eNif settings.
...
| `session.id` | Unique per eNif window |
...
To capture full content, add to your eNif settings:
...
The Copilot CLI SDK runs in the same eNif process and produces a rich trace hierarchy including subagents, permissions, hooks, and tool calls:
...
Then set the auth header via environment variable (required — no eNif setting for headers):
