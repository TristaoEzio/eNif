...
The current authentication mechanism is imperative and eNif-specific:

1. The renderer discovers agents via `listAgents()` and checks `IAgentDescriptor.requiresAuth`.
2. It obtains a GitHub OAuth token from eNif's built-in authentication service.
...
2. **Phase B**: Update eNif renderer to use `authenticate` instead of `setAuthToken`. External clients can start using the new flow.
