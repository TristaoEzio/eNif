...
export class StorageMainService extends Disposable implements IStorageMainService {
	...
	private createApplicationSharedStorage(): IStorageMain {
		...
		// In eNif: reuse the own application storage (keys are local)
		...
		if (hostUserRoamingDataHome) {
			// - In the Agents App: create a storage backed by the host (eNif)
			//   app's application DB so keys are found even if eNif hasn't
