...
6. Confirm the path is NOT the eNif profile folder (e.g., NOT `~/.vscode-oss-sessions-dev/User/...`)
...
- User file is created under `~/.copilot/instructions/` (not the eNif profile folder)
...
- The eNif profile folder should never be used for user creation in sessions
