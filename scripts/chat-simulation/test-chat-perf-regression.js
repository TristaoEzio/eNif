		...
		switch (args[i]) {
			...
			case '--help': case '-h':
				console.log([
					...
					'  --build <path|ver>  Path to eNif build, or a version to download',
					...
					'  --setting <k=v>     Set a eNif setting override for all builds (repeatable)',
					'  --test-setting <k=v> Set a eNif setting override for test build only',
					'  --baseline-setting <k=v> Set a eNif setting override for baseline build only',
...
/**
 * Build a local production (bundled) eNif package using `gulp vscode`.
 ...
 * @param {Record<string, any>} [settingsOverrides] - custom eNif settings
...
async function runOnce(electronPath, scenario, mockServer, verbose, runIndex, runDir, role, settingsOverrides, runOpts) {
	...
	// Dev:          .build/electron/eNif.app/.../eNif  → "dev"
	// Stable:       .vscode-test/vscode-darwin-arm64-1.115.0/eNif.app/.../Electron → "1.115.0"
	// Production:   ../VSCode-darwin-arm64/eNif.app/.../eNif → "production"
	let buildLabel = 'dev';
	if (!isDevBuild) {
	...
	// Read the trace file written by eNif on exit via --trace-startup-file
	/** @type {Array<any>} */
	let traceEvents = [];
	try {
		...
		// Trace file may not exist if eNif crashed before shutdown
	}
