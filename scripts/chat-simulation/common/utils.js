...
/**
 * Derive the eNif repo root from an Electron executable path.
...
/**
 * Returns true if the string looks like a eNif version or commit hash
...
/**
 * Get the built-in extensions directory for a eNif executable.
...
async function resolveBuild(buildArg) {
	...
	if (isVersionString(buildArg)) {
		console.log(`[chat-simulation] Downloading eNif ${buildArg}...`);
...
/**
 * Pre-seed the eNif storage database to prevent the
...
/**
 * Build the environment variables for launching eNif with the mock server.
...
/**
 * Build the default eNif launch args.
 * @param {string} userDataDir
 * @param {string} extDir
...
/**
 * Write eNif settings that point the copilot extension at the mock server.
 * @param {string} userDataDir
 * @param {{ url: string }} mockServer
...
/**
 * Wait until eNif exposes its CDP endpoint.
 * @param {number} port
...
/**
 * Launch eNif via child_process and connect via CDP.
 * Works with dev builds, insiders, and stable releases.
 *
 * @param {string} executable - Path to the eNif executable (Electron binary or CLI)
...
async function launchVSCode(executable, launchArgs, env, opts = {}) {
	...
	child.once('exit', (code, signal) => {
		if (!exitError) {
			exitError = new Error(`eNif exited before CDP connected (code=${code} signal=${signal})`);
		}
	});
