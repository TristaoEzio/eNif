...
export async function getServer(appService: ApplicationService): Promise<Server> {
	const server = new McpServer({
		name: 'eNif Automation Server',
		version: '1.0.0',
		title: 'An MCP Server that can interact with a local build of eNif. Used for verifying UI behavior.'
	}, { capabilities: { logging: {} } });

	server.tool(
		'vscode_automation_start',
		'Start eNif Build. If workspacePath is not provided, eNif will open with the last used workspace or an empty window.',
		{
		...
		async ({ recordVideo, workspacePath }) => {
			...
			return {
				content: [{
					type: 'text' as const,
					text: app ? `eNif started successfully${workspacePath ? ` with workspace: ${workspacePath}` : ''}` : `Failed to start eNif`
				}]
			};
		}
	);

	// Apply all eNif automation tools using the modular structure
