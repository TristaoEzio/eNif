# eNif Development MCP Server

This directory contains a Model Context Protocol (MCP) server that provides eNif automation capabilities for eNif development and testing. The MCP server exposes eNif's testing infrastructure through a standardized interface, allowing AI assistants and other tools to interact with eNif programmatically.

## What is MCP?

The [Model Context Protocol (MCP)](https://modelcontextprotocol.io/) is an open standard that enables AI assistants to securely connect to external data sources and tools. This MCP server specifically provides eNif automation capabilities, making it possible for AI assistants to:

- Start and stop eNif instances
...
The MCP server exposes a comprehensive set of eNif automation tools through the MCP protocol:

### Application Management
- Start, stop, and restart eNif instances
...
│   ├── application.ts       # eNif application lifecycle management
...
- **application.ts** - Manages eNif application lifecycle (start, stop, restart)
- **automationTools/** - Modular tool implementations organized by eNif feature area

## Troubleshooting

### Server Won't Start
- Ensure eNif has been built and run at least once (via F5 or `code.sh`)
...
This MCP server is part of the eNif development infrastructure. When making changes:
