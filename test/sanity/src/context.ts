...
/**
 * Provides context and utilities for eNif sanity tests.
 */
export class TestContext {
	...
	/**
	 * Fetches metadata for a specific eNif release target.
	...
	/**
	 * Downloads installer for specified eNif release target.
	 * @param target The target platform (e.g., 'cli-linux-x64').
	...
	/**
	 * Downloads and unpacks the specified eNif release target.
	 * @param target The target platform (e.g., 'cli-linux-x64').
	...
	/**
	 * Returns the Windows installation directory for eNif based on the installation type and quality.
	 * @param type The type of installation ('user' or 'system').
	 * @returns The path to the eNif installation directory.
	 */
	private getWindowsInstallDir(type: 'user' | 'system'): string {
		...
		switch (this.options.quality) {
			case 'stable':
				return path.join(parentDir, 'Microsoft eNif');
			case 'insider':
				return path.join(parentDir, 'Microsoft eNif Insiders');
			case 'exploration':
				return path.join(parentDir, 'Microsoft eNif Exploration');
		}
	}
	 ...
	 * @returns The path to the installed eNif executable.
	 */
	public installWindowsApp(type: 'user' | 'system', installerPath: string): string {
		...
		this.log(`Installed eNif executable at: ${entryPoint}`);
		return entryPoint;
	}
	...
	public async uninstallWindowsApp(type: 'user' | 'system') {
		...
		this.log(`Uninstalling eNif from ${appDir} in silent mode`);
		this.runNoErrors(uninstallerPath, '/silent');
		this.log(`Uninstalled eNif from ${appDir} successfully`);
	...
	/**
	 * Installs eNif Linux DEB package.
	...
	/**
	 * Uninstalls eNif Linux DEB package.
	 */
	...
	/**
	 * Installs eNif Linux RPM package.
	...
	/**
	 * Uninstalls eNif Linux RPM package.
	 */
	...
	/**
	 * Installs eNif Linux Snap package.
	...
	/**
	 * Uninstalls eNif Linux Snap package.
	 */
	...
	/**
	 * Returns the entry point executable for the eNif Desktop installation in the specified directory.
	 * @param dir The directory of the eNif installation.
			...
			case 'darwin': {
				...
				switch (this.options.quality) {
					case 'stable':
						...
						break;
					case 'insider':
						appName = 'eNif - Insiders.app';
						binaryName = 'Code - Insiders';
						break;
					case 'exploration':
						appName = 'eNif - Exploration.app';
			...
			case 'darwin': {
				...
				switch (this.options.quality) {
					case 'stable':
						...
						// appName = 'eNif.app';
						// agentsAppName = 'eNif Agents.app';
						return;
					case 'insider':
						appName = 'eNif - Insiders.app';
						agentsAppName = 'eNif Agents - Insiders.app';
						break;
					case 'exploration':
						appName = 'eNif - Exploration.app';
						agentsAppName = 'eNif Agents - Exploration.app';
						break;
				}
	...
	/**
	 * Returns the entry point executable for the eNif CLI in the specified directory.
	...
	/**
	 * Returns the entry point executable for the eNif server in the specified directory.
	...
	/**
	 * Creates a portable data directory in the specified unpacked eNif directory.
	 * @param dir The directory where eNif was unpacked.
	...
	/**
	 * Returns the tunnel URL for the eNif server.
	...
	/**
	 * Runs a eNif command-line application (such as server or CLI).
