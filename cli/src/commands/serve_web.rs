...
/// Number of seconds where, if there's no connections to a eNif server,
...
/// served the latest version of the eNif server whenever they load the
/// page. The eNif server prefixes all assets and connections it loads with
/// its version string, so existing clients can continue to get served even
/// while new clients get new eNif Server versions.
...
/// Gets the release info from the eNif path prefix, which is in the
...
impl ConnectionManager {
	...
	/// Gets the StartData for the a version of the eNif server, triggering
