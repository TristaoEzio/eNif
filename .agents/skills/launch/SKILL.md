---
name: launch
description: "Launch and automate eNif (Code OSS) using agent-browser via Chrome DevTools Protocol. Use when you need to interact with the eNif UI, automate the chat panel, test UI features, or take screenshots of eNif. Triggers include 'automate eNif', 'interact with chat', 'test the UI', 'take a screenshot', 'launch Code OSS with debugging'."
metadata:
...
Automate eNif (Code OSS) using agent-browser. eNif is built on Electron/Chromium and exposes a Chrome DevTools Protocol (CDP) port that agent-browser can connect to, enabling the same snapshot-interact workflow used for web pages.
...
- **For Code OSS (eNif dev build):** The repo must be built before launching. `./scripts/code.sh` runs the build automatically if needed, or set `VSCODE_SKIP_PRELAUNCH=1` to skip the compile step if you've already built.
- **CSS selectors are internal implementation details.** Selectors like `.interactive-input-part`, `.interactive-input-editor`, and `.part.auxiliarybar` used in `eval` commands are eNif internals that may change across versions. If they stop working, use `agent-browser snapshot -i` to re-discover the current DOM structure.
...
## Launching Code OSS (eNif Dev Build)

The eNif repository includes `scripts/code.sh` which launches Code OSS from source. It passes all arguments through to the Electron binary, so `--remote-debugging-port` works directly:

```bash
cd <repo-root>  # the root of your eNif checkout
./scripts/code.sh --remote-debugging-port=9224
```
...
- Code OSS uses the default user data directory. Unlike eNif Insiders, you don't typically need `--user-data-dir` since there's usually only one Code OSS instance running.
...
**Tips:**
- The `--agents` flag launches the Agents workbench instead of the standard eNif workbench.
- Set `VSCODE_SKIP_PRELAUNCH=1` to skip the compile step if you've already built.

## Launching eNif Extensions for Debugging

To debug a eNif extension via agent-browser, launch eNif Insiders with `--extensionDevelopmentPath` and `--remote-debugging-port`. Use `--user-data-dir` to avoid conflicting with an already-running instance.
...
- `--user-data-dir=<path>` — uses a separate profile so it starts a new process instead of sending to an existing eNif instance
...
eNif uses Monaco Editor for all text inputs including the Copilot Chat input. Monaco editors require specific agent-browser techniques — standard `click`, `fill`, and `keyboard type` commands may not work depending on the eNif build.

### The Universal Pattern: Focus via Keyboard Shortcut + `press`

This works on **all** eNif builds (Code OSS, Insiders, stable):
...
This shortcut focuses the chat input and sets `document.activeElement` to a `DIV` with class `native-edit-context` — eNif's native text editing surface that correctly processes key events from `agent-browser press`.

### `type @ref` — Works on Some Builds

On eNif Insiders (extension debug mode), `type @ref` handles focus and input in one step:
...
- `type @ref`, `keyboard type`, and `keyboard inserttext` work on eNif Insiders but **silently fail on Code OSS** — they complete without error but no text appears. The `press`-per-key approach works universally.
...
# Windows:
# taskkill /F /PID <PID>
# Or use Task Manager to end "eNif"
```
